...
void MyActuator::executeCommand(const MotorCommand& cmd)
{
    ...
    // ── Brake ───────────────────────────────────────────────────────────────
    case CommandType::BRAKE_LOCK:
        driver_->Motor_brake_lock(0XCD);
        break;

    case CommandType::BRAKE_RELEASE:
        driver_->Motor_brake_release(0XCD);
        break;
    ...
    case CommandType::READ_STATE1:
    {
        ...
        if (driver_->Motor_state1(cmd.motorId, data) == 0)
        {
            ...
            {
                std::lock_guard<std::mutex> cbLk(cbMutex_);
                if (faultCb_) faultCb_(cmd.motorId, s.errorState);
            }
        }
        break;
    }
...
void MyActuator::brakeLock(uint8_t motorId)
{
    if (!validId(motorId)) return;
    PrioritisedCommand pc;
    pc.cmd.type    = CommandType::BRAKE_LOCK;
    pc.cmd.motorId = motorId;
    std::lock_guard<std::mutex> lk(cmdMutex_);
    cmdQueue_.push(pc);
}

void MyActuator::brakeRelease(uint8_t motorId)
{
    if (!validId(motorId)) return;
    PrioritisedCommand pc;
    pc.cmd.type    = CommandType::BRAKE_RELEASE;
    pc.cmd.motorId = motorId;
    std::lock_guard<std::mutex> lk(cmdMutex_);
    cmdQueue_.push(pc);
}
...
{
    if (!validId(motorId)) return false;
